...
class AdminController extends Controller {
    ...
    async login(req, res) {
        ...
        const { token, admin } = await this.service.authenticate(email, password);
        res.cookie('auth', token);
        return HttpResponses.ok(res, { admin }, 'login successful');
    }
    ...
    async post(req, res) {
        const admin = await this.service.create(req.body, { file: req.file });
        await this.service.afterCreate(admin);
        return HttpResponses.created(res, admin, 'invitation sent successfully');
    }
    ...
    async uploadPicture(req, res) {
        const admin = await this.service.uploadPicture(
            req.params.id,
            req.file
        );
        return HttpResponses.ok(res, admin, 'picture updated successfully');
    }
}
