...
export const schemas = {
    ...
    merge: z.object({
        ...
        references: z.array(database.id).min(1, 'debe indicar al menos una amenidad a absorber'),
    }).strict().refine(
        data => !data.references.includes(data.principal),
        { path: ['references'], message: 'la amenidad principal no puede aparecer también en las referencias' }
    ),
};