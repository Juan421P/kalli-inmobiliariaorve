...
/**
 * Pagina "Hacer una oferta" sobre una propiedad. Esta detras de
 * ProtectedRoute (ver App.jsx), asi que siempre hay un usuario logueado
 * cuando se renderiza. El formulario usa react-hook-form: los campos de
 * texto/numero se registran normal, y los botones tipo "chip" (duracion
 * de renta, metodo de contacto) se manejan via setValue dentro de
 * useOfferForm para que igual participen de la validacion.
 */
const MakeOfferPage = () => {
                    ...
                    <form onSubmit={handleSubmit} className='grid grid-cols-1 md:grid-cols-2 gap-8 items-start'>
                        ...
                        <div className='bg-white/80 backdrop-blur-sm border border-white rounded-2xl p-6 flex flex-col gap-6 shadow-sm'>
                            ...
                            <div className='flex flex-col gap-1.5'>
                                ...
                                {errors.price && (
                                    <p className='text-[11px] text-red-500'>Ingresá un monto de oferta válido.</p>
                                )}
                                {property?.price && (
                                            ...
                                            <div className='grid grid-cols-4 gap-2'>
                                                {RENTAL_OPTIONS.map((opt) => (
                                                    <button
                                                        type='button'
                                                        key={opt.value}
                                                        // click de nuevo sobre la opcion ya elegida = deseleccionar
