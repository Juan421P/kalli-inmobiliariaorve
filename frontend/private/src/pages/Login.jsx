...
import { login } from '@/schemas/admin'
...
const Login = () => {
    ...
    const { control, handleSubmit, formState: { errors } } = useForm({ resolver: zodResolver(login) })
    ...
    const attemptLogin = async (data) => {
        ...
        try {
            ...
            if (adminResponse.token) localStorage.setItem('admin_token', adminResponse.token)
            toast.success('Inicio de sesión exitoso ☺️')
            navigate('/dashboard')
        } catch (adminError) {
            try {
                const collabResponse = await CollaboratorService.login(data.email, data.password)
                if (collabResponse.token) localStorage.setItem('collaborator_token', collabResponse.token)
                toast.success('Inicio de sesión exitoso ☺️')
                navigate('/dashboard')
            } catch (collabError) {
                toast.error(collabError.response?.data?.message || adminError.response?.data?.message || 'Credenciales inválidas')
            }
        } finally {
            setIsLoading(false)
        }
                    ...
                    <form onSubmit={handleSubmit(attemptLogin)} noValidate className='space-y-6'>
                                    ...
                                    <TooltipProvider>
                                        <Tooltip open={!!errors.email}>
                                            <TooltipTrigger asChild>
                                                <Input id='email' type='email' placeholder='Ingrese su correo electrónico' value={value || ''} onBlur={onBlur} ref={ref} onChange={(e) => { const filtered = filterInvalidEmailChars(e.target.value); onChange(filtered); }} className={cn('pl-12 bg-orve-teal/20 border-transparent h-12 rounded-xl text-orve-teal placeholder:text-orve-teal/75 placeholder:italic drop-shadow-md font-medium transition-colors', errors.email && 'border-orve-red')} />
                                            </TooltipTrigger>
                        ...
                        <div className='space-y-1'>
                            ...
                            <div className='relative'>
                                ...
                                <Controller control={control} name='password' render={({ field: { onChange, onBlur, value, ref } }) => (
                                    <TooltipProvider>
                                        <Tooltip open={!!errors.password}>
                                            <TooltipTrigger asChild>
                                                <Input id='password' type={showPassword ? 'text' : 'password'} placeholder='Ingrese su contraseña' value={value || ''} onBlur={onBlur} ref={ref} onChange={(e) => { const filtered = filterInvalidPasswordChars(e.target.value); onChange(filtered); }} className={cn('pl-12 pr-12 bg-orve-teal/20 border-transparent h-12 rounded-xl text-orve-teal placeholder:text-orve-teal/75 placeholder:italic drop-shadow-md font-medium transition-colors', errors.password && 'border-orve-red')} />
                                            </TooltipTrigger>
                                            <TooltipContent side="right" className="bg-orve-red text-white font-semibold border-none shadow-md">
                        ...
                        <div className='flex items-center justify-between pl-2'>
                            <div className='flex items-center space-x-2'>
                                <Checkbox id='remember' className='bg-orve-teal/20 data-[state=checked]:bg-orve-teal' />
                            ...
                            <a href='#' className='pl-2 text-sm text-orve-teal/40 font-medium cursor-pointer hover:text-orve-teal/80 transition-colors select-none'>¿Olvidó su contraseña?</a>
                        </div>
