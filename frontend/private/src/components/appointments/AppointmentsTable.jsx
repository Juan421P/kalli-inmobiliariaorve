...
import AssignCollaboratorDialog from './assignCollaboratorDialog'
import ScheduleAppointmentDialog from './scheduleAppointmentDialog'
import { cn } from '@/lib/utils'
import { getStatus, formatDate } from './constants'

const AppointmentsTable = ({
    appointments = [], isLoading, isSubmitting, isAdmin,
    onEdit, onComplete, onCancel, onAssign, onSchedule,
}) => {
    if (isLoading) {
        return (
        ...
        <Table>
            ...
            <TableBody>
                {appointments.map((apt) => {
                    ...
                    // ofrecer ninguna acción sobre una cita cerrada
                    const isOpen = apt.status === 'pending' || apt.status === 'assigned' || apt.status === 'scheduled'
                    // el backend solo deja asignar/agendar en estos estados puntuales
                    // (ver appointment.js: assign requiere pending o assigned, schedule requiere assigned)
                    const canAssign = isAdmin && (apt.status === 'pending' || apt.status === 'assigned')
                    const canSchedule = apt.status === 'assigned'
                        ...
                        <TableRow key={apt._id} className='border-orve-teal/10 hover:bg-orve-teal/[0.03]'>
                            ...
                            <TableCell className='text-right'>
                                <Popover>
                                    ...
                                    <PopoverContent align='end' className='w-48 p-1 bg-white border border-input shadow-md'>
                                        ...
                                        {isOpen && (
                                            <>
                                                ...
                                                {canAssign && (
                                                    <AssignCollaboratorDialog appointment={apt} onAssign={onAssign} isLoading={isSubmitting} />
                                                )}
                                                {canSchedule && (
                                                    <ScheduleAppointmentDialog appointment={apt} onSchedule={onSchedule} isLoading={isSubmitting} />
                                                )}
                                                {canComplete && (
                                                    <button
